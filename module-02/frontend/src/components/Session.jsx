...
import { candidateInvitationUrl, copyInvitation } from '../invitation.js';
...
export default function Session({ id, token }) {
  ...
  const candidateUrl = state.candidateLink ? candidateInvitationUrl(state.candidateLink, window.location.origin) : '';
  function edit(field, value) {
    try { connection.current[field](value); } catch (err) { setError(err.message); }
  }
  async function copy() {
    const copied = await copyInvitation(invitation.current);
    setCopyStatus(copied ? 'Link copied' : 'Automatic copy failed. Select and copy the invitation link manually.');
  }
  return <main className="session">
    ...
    {interviewer && <div className="invitation"><label htmlFor="invitation">Candidate invitation</label><input ref={invitation} id="invitation" readOnly value={candidateUrl} onFocus={event => event.target.select()} /><span role="status">{copyStatus}</span><small>Keep your own address to return as interviewer. Both links are private.</small></div>}
