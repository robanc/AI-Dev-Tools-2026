...
export default function Session({ id, token }) {
  ...
  useEffect(() => {
    try { connection.current = interviewService.joinSession(id, token, setState); }
  ...
  if (!state) return <main><p role="status">Opening session…</p></main>;
  ...
  return <main className="session">
    ...
    <details className="prototype"><summary>Prototype connection controls</summary><p>Sessions are shared between tabs in this browser on this site. Use an external call for conversation.</p><button className="secondary" onClick={() => state.connected ? connection.current.disconnect() : connection.current.reconnect()}>{state.connected ? 'Simulate disconnection' : 'Reconnect'}</button></details>
  </main>;
}
