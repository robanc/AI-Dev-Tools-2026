...
export default function App() {
  ...
  async function create() {
    ...
    catch { setError('Could not create a session. Allow browser storage and try again.'); }
    finally { setBusy(false); }
  }
  const match = hash.match(/^#\/session\/([^/]+)\/([^/]+)$/);
  const home = !hash || hash === '#/' || hash === '#';
  return <><header><a className="brand" href="#/"><span aria-hidden="true">⌘</span> pairroom<span className="brand-dot">.</span></a><span className="header-note">A little space for big ideas <span className="prototype-badge">Prototype</span></span></header>
    {home ? <main className="home"><span className="eyebrow">One problem. Two perspectives.</span><h1>Good interviews<br />start with a <em>shared space.</em></h1><p className="intro">A focused room for your next coding conversation. Share a problem, watch a solution take shape, and think it through together.</p><button disabled={busy} onClick={create}>{busy ? 'Creating…' : 'Create session'} <span aria-hidden="true">↗</span></button><p className="hint">No accounts. Just a private invitation link.</p>{error && <p role="alert" className="notice">{error}</p>}<div className="steps"><div><span>01 / CREATE</span><h2>Make room.</h2><p>Start a session and write your problem.</p></div><div><span>02 / INVITE</span><h2>Bring someone in.</h2><p>Copy the candidate link into another tab.</p></div><div><span>03 / COLLABORATE</span><h2>Think together.</h2><p>Follow edits as they happen. Talk on your own call.</p></div></div><p className="mock-note">Frontend preview · Sessions stay in this browser. Open both links on the same site in the same browser profile.</p></main>
